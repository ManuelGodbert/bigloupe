			...
			function(action, el, pos) {
					if (action == 'disable' ) {
						...
						if (selection.length > 0) {
							...
							window.location.href= contextURL + "/scheduler/jobDetail.html?id=" + id;
						}
					}
			});
		}
...
$(function () {
	...
	$("#jobsearch").keypress(function(event, item) {
		  if ( event.which == 13 ) {
			  if (currentGraph) {
				  currentGraph.selectAndCenter(item);
		   }
...
function executeFlow() {
	...
	jQuery.ajax( {
		'type': 'POST',
		'url': contextURL + "/scheduler/subflow.html", 
		'async':false,
		'data': {
			"id":flowID,
			"name": name,
				...
				$("#modelDialog").dialog(
					{
						...
						close: function(event, ui) {
							window.location.href = contextURL + "/scheduler/subflow.html?id=" + data.id;
						}
					}
				).position({
				       my: 'left',
				...
				$("#modelDialog").dialog(
					{
						...
						close: function(event, ui) {
							if (data.id) {
								...
								window.location.href = contextURL + "/scheduler/flow.html?job_id=" + name;
							}
						}
					}
				);
