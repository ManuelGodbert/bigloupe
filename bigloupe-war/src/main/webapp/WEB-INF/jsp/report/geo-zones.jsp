...
<div class="row">
	<div class="page-header">
		<h3>
			Geographical Zones<small>&nbsp;Drag to rotate the origin</small>
		</h3>
...
<script type="text/javascript">
	...
	var projection = d3.geo.azimuthal().scale(380).origin([ -71.03, 42.37 ])
			.mode("orthographic").translate([ 640, 400 ]);
	...
	var svg = d3.select("#body").append("svg:svg").attr("width", 1280).attr(
			"height", 800).on("mousedown", mousedown);

	d3.json("${request.contextPath}/data/json/world-countries.json", function(
			collection) {
	...
	function mousemove() {
		if (m0) {
			var m1 = [ d3.event.pageX, d3.event.pageY ], o1 = [
					o0[0] + (m0[0] - m1[0]) / 8, o0[1] + (m1[1] - m0[1]) / 8 ];
	...
	function refresh(duration) {
		(duration ? feature.transition().duration(duration) : feature).attr(
				"d", clip);
	}
