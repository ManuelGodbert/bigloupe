package org.bigloupe.web.filemanager.config;
...
public abstract class AbstractConnectorConfig {
	private static final String[] allowedCommands = new String[] { "content",
			"init", "mkdir", "mkfile", "open", "rename", "rm", "upload",
			"paste" };
	...
	/**
	 * Just a common implementation of basic permission check, allowing to do
	 * everything on all files except deleting the root directory.
	 * 
	 * @param dir
	 * @param action
	 * @return
	 ...
	 *            $name file name
	 * @param string
	 *            $tmpName uploaded file tmp name
	 * @return bool
	 **/
	...
	/**
	 * Return true if file name is not . or .. If file name begins with . return
	 * value according to $this->_options['dotFiles']
	 * 
	 * @param string
	 *            $file file name
	 * @return bool
