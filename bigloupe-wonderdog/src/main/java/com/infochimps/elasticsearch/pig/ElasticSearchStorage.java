...
public class ElasticSearchStorage extends LoadFunc implements
		StoreFuncInterface {
	...
	// CHANGE
	// private static final String ES_CONFIG_HDFS_PATH =
	// "/tmp/elasticsearch/elasticsearch.yml";
	// private static final String ES_PLUGINS_HDFS_PATH =
	// "/tmp/elasticsearch/plugins";

	private static final String ES_CONFIG_HDFS_PATH = "/usr/karma/index/search.yml";
	private static final String ES_PLUGINS_HDFS_PATH = "/usr/karma/index/plugins";
	...
	public OutputFormat getOutputFormat() throws IOException {
		return new ElasticSearchOutputFormat(false);
	}
	...
	private void elasticSearchSetup(String location, Job job) {
		...
		try {
			...
			// Parse the passed in location URI, pulling out the arguments as
			// well
			URI parsedLocation = new URI(location);
			HashMap<String, String> query = parseURIQuery(parsedLocation
					.getQuery());

			String esHost = location.substring(5).split("/")[0];
			if (esHost == null) {
				throw new RuntimeException(
						"Missing elasticsearch index name, URI must be formatted as es://<index_name>/<object_type>?<params>");
			}

			if (parsedLocation.getPath() == null) {
				throw new RuntimeException(
						"Missing elasticsearch object type, URI must be formatted as es://<index_name>/<object_type>?<params>");
			}

			Configuration conf = job.getConfiguration();
			if (conf.get(ES_INDEX_NAME) == null) {

				// Set elasticsearch index and object type in the Hadoop
				// configuration
				job.getConfiguration().set(ES_INDEX_NAME, esHost);
				job.getConfiguration().set(ES_OBJECT_TYPE,
						parsedLocation.getPath().replaceAll("/", ""));
				...
				// Adds the elasticsearch.yml file (esConfig) and the plugins
				// directory (esPlugins) to the distributed cache
//				try {
//					Path hdfsConfigPath = new Path(ES_CONFIG_HDFS_PATH);
//					Path hdfsPluginsPath = new Path(ES_PLUGINS_HDFS_PATH);
//
//					HadoopUtils
//							.uploadLocalFileIfChanged(new Path(LOCAL_SCHEME
//									+ esConfig), hdfsConfigPath,
//									job.getConfiguration());
//					HadoopUtils.shipFileIfNotShipped(hdfsConfigPath,
//							job.getConfiguration());
//
//					HadoopUtils.uploadLocalFileIfChanged(new Path(LOCAL_SCHEME
//							+ esPlugins), hdfsPluginsPath,
//							job.getConfiguration());
//					HadoopUtils.shipArchiveIfNotShipped(hdfsPluginsPath,
//							job.getConfiguration());
//
//				} catch (Exception e) {
...
//				}
	 ...
	 * critical to ensure that we do not change or reset anything we've already
	 * set.
	 */
	...
	/**
	 * Given a URI query string, eg. "foo=bar&happy=true" returns a hashmap
	 * ({'foo' => 'bar', 'happy' => 'true'})
	 */
	...
	private Writable toWritable(Object thing) {
		if (thing instanceof String) {
			...
			for (Map.Entry<String, Object> entry : ((Map<String, Object>) thing)
					.entrySet()) {
				result.put(new Text(entry.getKey().toString()),
						toWritable(entry.getValue()));
			}
			return result;
			...
			if (((List) thing).size() > 0) {
				...
				return new ArrayWritable(toWritable(first).getClass(),
						listOfThings);
			}
		}
