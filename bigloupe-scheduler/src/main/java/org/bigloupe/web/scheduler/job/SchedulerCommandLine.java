...
import org.bigloupe.web.scheduler.PropsUtils;
import org.bigloupe.web.util.Props;


public class SchedulerCommandLine {

	private static Logger logger = Logger.getLogger(SchedulerCommandLine.class);
	...
	public SchedulerCommandLine(String[] args) {
		this(new OptionParser(), args);
	}

	public SchedulerCommandLine(OptionParser parser, String[] args) {
		this.parser = parser;
		...
		configDirOpt = parser
				.acceptsAll(asList("c", "config-dir"),
						"A configuration directory for jobs, if separate from the job directory.")
				.withRequiredArg().describedAs("dir");
