...
public class JobExecutorManager {
    ...
    private void sendErrorEmail(JobExecution job,
        ...
        if(emailList != null && mailerService != null) {
            try {
                ...
                int errorNo = 1;
                String logUrlPrefix = runtimeProps != null ? runtimeProps.getProperty(BigLoupeConfiguration.DEFAULT_LOG_URL_PREFIX)
                                                           : null;
                if(logUrlPrefix == null && runtimeProps != null) {
                    logUrlPrefix = runtimeProps.getProperty(BigLoupeConfiguration.LOG_URL_PREFIX);
                }
                ...
                mailerService.sendEmailIfPossible(senderAddress,
                                             emailList,
    ...
    private Props produceParentProperties(final ExecutableFlow flow) {
        ...
        // Add properties now
        parentProps.put("now", loadTime.toString("yyyy") + loadTime.toString("MM") + loadTime.toString("dd"));
        return parentProps;
    }
