...
import org.bigloupe.web.scheduler.job.AbstractProcessJob;
...
public class ProcessJob extends AbstractProcessJob implements Job {
    ...
    public void run() {
        ...
        for(String command: commands) {
            ...
            // Save command in a bash
            File commandFile = new File(getWorkingDirectory(), getId() + ".cmd");
            try {
				FileUtils.write(commandFile, command);
				info("Save command line in file " + commandFile);
			} catch (IOException e1) {
				info("Can't save command line in file " + commandFile);
			}
