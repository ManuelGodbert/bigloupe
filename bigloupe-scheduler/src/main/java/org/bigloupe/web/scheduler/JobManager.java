...
public class JobManager {
	...
	private static final DateTimeFormatter JOB_EXEC_DATE_FORMAT = DateTimeFormat
			.forPattern("MM-dd-yyyy.HH.mm.ss");
	private static final Set<String> EXCLUDE_PATHS = ImmutableSet
			.of("__MACOSX");
	private static final Comparator<JobExecution> JOB_EXEC_COMPARATOR = new Comparator<JobExecution>() {
		...
		public int compare(JobExecution e1, JobExecution e2) {
			long ended1 = e1.getStarted() == null ? Long.MIN_VALUE : e1
					.getStarted().getMillis();
			long ended2 = e2.getStarted() == null ? Long.MIN_VALUE : e2
					.getStarted().getMillis();
	...
	private final BigLoupeConfiguration configuration;

	private static Logger logger = Logger.getLogger(JobManager.class);

	private volatile FlowManager manager;
	private final AtomicReference<Map<String, JobDescriptor>> jobDescriptorCache = new AtomicReference<Map<String, JobDescriptor>>(
			Collections.<String, JobDescriptor> emptyMap());

	public JobManager(final JobWrappingFactory factory, final String logDir,
			final Props defaultProps, final List<File> jobDirs,
			final ClassLoader classLoader, BigLoupeConfiguration configuration) {
	 ...
	 */
	public Job loadJob(String jobName, Props overrides,
			boolean ignoreDependencies) {
		Map<String, JobDescriptor> descriptors = loadJobDescriptors(overrides,
				new HashMap<File, File>(), ignoreDependencies);
		JobDescriptor desc = descriptors.get(jobName);
		if (desc == null)
			throw new JobLoadException("No job descriptor found for job '"
					+ jobName + "'.");
	 ...
	 */
	private Job loadJob(JobDescriptor desc, Map<String, Job> loadedJobs,
			boolean ignoreDependencies) {
	 ...
	 */
	public Set<JobDescriptor> getRootJobDescriptors(
	...
	public List<JobExecution> loadRecentJobExecutions(int count)
			throws IOException {
		...
		for (File jobDir : jobDirs) {
			List<JobExecution> found = loadJobExecutions(jobDir.getName(),
					jobDir);
			if (found.size() > count)
	 ...
	 *             If loading fails
	 */
	public List<JobExecution> loadJobExecutions(String jobName)
			throws IOException {
	...
	private List<JobExecution> loadJobExecutions(String jobName, File jobDir)
			throws IOException {
		List<JobExecution> execs = new ArrayList<JobExecution>();
		File[] files = jobDir.listFiles();
		if (files != null) {
			for (File execDir : files) {
				// Sometimes there are os or other maintenance files that should
				// be skipped.
				...
				try {
					dirDate = JOB_EXEC_DATE_FORMAT.parseDateTime(execDir
							.getName());
				} catch (Exception e) {
					logger.info("Ignoring unknown directory found in logs:"
							+ execDir.getAbsolutePath(), e);
					continue;
				}
				if (runProps.canRead()) {
				...
				String logFile = jobName + File.separator + execDir.getName()
						+ File.separator + jobName + "." + execDir.getName()
						+ ".log";
				execs.add(new JobExecution(jobName, start == null ? dirDate
						: start, end, succeeded, false, logFile));
			}
		}
		return execs;
	}
	...
	public Map<String, JobDescriptor> loadJobDescriptors() {
		...
		Props overrides = addGenericProperties();
		return loadJobDescriptors(overrides, new HashMap<File, File>(), false);
	}
	 ...
	 * @return
	 */
	private Props addGenericProperties() {
	...
	public Map<String, JobDescriptor> loadJobDescriptors(Props overrides,
		...
		for (File file : _jobDirs) {
			Map<String, JobDescriptor> d = loadJobDescriptors(file, overrides,
					pathOverrides, ignoreDeps);
			for (Map.Entry<String, JobDescriptor> entry : d.entrySet()) {
				if (descriptors.containsKey(entry.getKey()))
					throw new IllegalStateException("Job " + entry.getKey()
							+ " already exists.");
	 ...
	 *            Properties that override all other properties
	 */
	protected Map<String, JobDescriptor> loadJobDescriptors(File jobDir,
			Props propsOverrides, Map<File, File> pathOverrides,
			boolean ignoreDeps) {
		if (!jobDir.exists() || !jobDir.isDirectory())
			throw new AppConfigurationException(jobDir
					+ " is not a readable directory.");
		Map<String, JobDescriptor> m = new HashMap<String, JobDescriptor>();
		loadJobDescriptorsWithoutDependencies(m, jobDir, jobDir, _defaultProps,
				propsOverrides, pathOverrides, _baseClassLoader);
		...
		if (pathOverrides != null) {
			...
			for (Map.Entry<File, File> override : pathOverrides.entrySet()) {
				loadJobDescriptorsWithoutDependencies(m, jobDir,
						override.getValue(), dirProps, propsOverrides,
	 ...
	 */
	private void loadJobDescriptorsWithoutDependencies(
			Map<String, JobDescriptor> jobs, File baseDir, File currDir,
			Props defaultProps, Props overrides, Map<File, File> pathOverrides,
		...
		for (File f : status) {
			if (EXCLUDE_PATHS.contains(f.getName())
				...
				if (pathOverrides != null && pathOverrides.containsKey(f))
					...
					loadJobDescriptorsWithoutDependencies(jobs, baseDir, f,
							dirProps, overrides, pathOverrides, loader);
			} else {
	...
	private String getJobPath(File baseDir, File currDir) {
		try {
			return currDir.getCanonicalPath().substring(
					baseDir.getCanonicalPath().length());
		} catch (IOException e) {
			throw new JobLoadException("Error while cannonicalizing job path.",
					e);
		}
	}
	 ...
	 * @param dir
	 *            The directory to look for jars in
	 * @return The classloader
	 */
	private ClassLoader createClassLoaderForDir(ClassLoader parentClassLoader,
			File dir) {
		...
		if (files == null) {
			...
			for (File f : files) {
				if (f.getName().endsWith(".jar")) {
					try {
						logger.debug("Adding jar " + f.getName()
								+ " to the classpath");
	...
	private void addDependencies(Map<String, JobDescriptor> jobs) {
		...
		for (JobDescriptor job : jobs.values()) {
			List<String> dependencies = job.getProps().getStringList(
	...
	// Add Hadoop configuration in BigLoupe Web for all jobs transparently
	private void addHadoopConfiguration(Map<String, JobDescriptor> jobs) {
		for (JobDescriptor job : jobs.values()) {
			String fullPath = job.getFullPath();

			// If current job has an hadoop cluster : use in priority this
			// cluster

			// hadoop.cluster property vs hadoop.home property priority
			// Hadoop.cluster set by BigLoupe Web 
			String hadoopCluster;
			if (job.getProps().containsKey("hadoop.cluster")) {
				hadoopCluster = fullPath.substring(0,
						fullPath.indexOf("schedule"))
						+ job.getProps().get("hadoop.cluster");

			} else if (job.getProps().containsKey("hadoop.home")) {
				// if hadoop.home is an absolute path, don't change it
				// if hadoop.home is a relative path, add full path
				boolean unixSeparator = (System.getProperty("file.separator").equals("/") ? true : false);
				hadoopCluster = FilenameUtils.normalize(job.getProps().get("hadoop.home"), unixSeparator);

				if (!hadoopCluster.startsWith("/") || !hadoopCluster.contains(":/")) {
					hadoopCluster = fullPath.substring(0,
							fullPath.indexOf("schedule")) + hadoopCluster;
				}
			...
			job.getProps().put("hadoop.home", hadoopCluster);
		...
		try {
			...
			for (File f : dir.listFiles()) {
				try {
					...
					throw new JobLoadException(
							"Undefined property while loading properties in '"
									+ f + "'.", e);
				}
			}
			return props;
		...
		synchronized (this) {
			if (manager == null) {
				...
				throw new IllegalStateException(
						"Can only set the FlowManager on JobManager onces.");
			}
		}
	...
	public void deployJobDir(String localPath, String destPath) {
		...
		// Keep job directory and deploy scripts only
		...
		try {
			FileUtils.copyDirectory(currPath, targetPath);
	...
	public void deployJob(String jobName, String path, Props props) {
		...
		if (jobPath.exists()) {
			if (!jobPath.isDirectory() || !jobPath.canWrite())
				throw new JobDeploymentException(
						jobPath
